...
if __name__ == '__main__':
    ...
    df_train_dicom_tags['scan_id'] = df_train_dicom_tags['path'].apply(lambda x: str(x).split('/')[2]).astype(np.uint64)
    df_train_dicom_tags['patient_id'] = df_train_dicom_tags['path'].apply(lambda x: str(x).split('/')[1]).astype(np.uint64)
    df_train_dicom_tags['slice_id'] = df_train_dicom_tags['path'].apply(lambda x: str(x).split('/')[3].split('.')[0]).astype(np.uint64)
    df_train_dicom_tags['z_position'] = df_train_dicom_tags['ImagePositionPatient'].apply(lambda x: eval(x)[-1]).astype(np.float32)
    df_train_dicom_tags = df_train_dicom_tags.sort_values(by=['patient_id', 'scan_id', 'z_position'], ascending=True).reset_index(drop=True)
    df_train_dicom_tags['z_position_diff'] = df_train_dicom_tags.groupby(['patient_id', 'scan_id'])['z_position'].diff().round(2)
    df_train_dicom_tags['slice_id_diff'] = df_train_dicom_tags.groupby(['patient_id', 'scan_id'])[['slice_id']].diff()
    df_train_dicom_tags['image_path'] = df_train_dicom_tags['path'].apply(lambda x: str(train_directory) + '/' + x)
    ...
    for patient_id, scan_id in zip(patient_ids, scan_ids):
        ...
        df_scan = df_train_dicom_tags.loc[(df_train_dicom_tags['patient_id'] == patient_id) & (df_train_dicom_tags['scan_id'] == scan_id)].reset_index(drop=True)
        ...
        for dicom_file_path in tqdm(df_scan['image_path'].values):
            ...
            image = dicom_utilities.adjust_pixel_values(
                ...
                window_center=50, window_width=400,
